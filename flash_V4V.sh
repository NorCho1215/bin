...
if [ "$#" -lt 2 ]; then
  ...
  if [ "$image_bind" == "HA" ]; then
      ...
      make
      cd ../../../../
  elif [ "$image_bind" == "CCC" ]; then
      bind_partitions linux-qnx-android-qnx-qnx -b p3407-t186
      cd hypervisor/hypervisor-a15/tools/uart_muxer
      make
      cd ../../../../
  fi
...
#===============================
  ...
  if [ "$image_bind" == "A" ]; then
      ...
      ANDROID_BUILD_FLAVOR=release ANDROID_TARGET_DEVICE=vcm31t186 eenv ./embedded/tools/scripts/t186_bootburn/bootburn.sh -b p3407-t186 $parameter
  else
