...
if [ "$#" -lt 2 ]; then
    echo "command: /home/ncho/bin/flash_V4A_GVS_cust.sh <folder> <release/debug> <board- ES/INT>"
else
  test -f customerBuildOutput.release.tgz && mv customerBuildOutput.release.tgz output.android.tgz
  test -f output.tgz &&  mv output.tgz output.foundation.tgz
  mkdir $folder
  cd $folder
