...
#===============================
  ...
  if [ "$image_bind" == "A" ]; then
      ANDROID_BUILD_FLAVOR=release ANDROID_TARGET_DEVICE=t186 flash -a -b p2382-t186
  elif [ "$image_bind" == "L" ]; then
      ANDROID_BUILD_FLAVOR=release flash -b p2382-t186
  else
      if [ "$partation_name" == "" ]; then
          ANDROID_BUILD_FLAVOR=release ANDROID_TARGET_DEVICE=t186 flash $parameter -b p2382-t186
      else
          ANDROID_BUILD_FLAVOR=release ANDROID_TARGET_DEVICE=t186 flash $parameter -b p2382-t186 -u $partation_name
      fi
  fi
